...
- **Ogni commit = tag `vX.Y.Z` annotato**, senza chiederlo: `minor` per novita sostanziali (nuovi template/comandi/feature, rimozioni), `patch` per fix/bump dipendenze. Il messaggio del tag e la voce di changelog: cosa cambia e perche, non l'elenco dei file. La versione del binario arriva da `-ldflags "-X main.version=..."` (il workflow la prende da `github.ref_name`): **non** hardcodarla in `main.go`. Resta da allineare a mano `version` in `extensions/vscode/package.json`.
...
- **A decidere se pubblicare l'estensione e la `version`, non il tag.** Qualunque tag fa da innesco, ma il job `decide` interroga la gallery pubblica e pubblica **solo se quella versione non e gia online**. Questo e cio che rende compatibile "un tag per commit" con dei marketplace dove le versioni sono **immutabili** e notificano gli utenti: ripubblicare la stessa fallirebbe, pubblicarne una nuova a ogni commit sarebbe spam. Quindi per rilasciare l'estensione basta **alzare `version` in `extensions/vscode/package.json`**: il tag successivo la pubblica, quelli dopo non fanno nulla. Senza il secret `VSCE_PAT` (e `OVSX_PAT`, opzionale) gli step di publish vengono **saltati**, non falliscono.
...
- **La versione della CLI non e un indicatore affidabile**: il binario pre-riscrittura hardcodava `Version: "v1.0.1"` in `main.go`, cioe un numero **piu alto** di quelli che hanno corretto il bug. Un `go install` vecchio lascia quel binario in `~/go/bin/Go-DDD-Scaffold` e un controllo di versione minima lo farebbe passare. Per distinguere una CLI utilizzabile serve un **probe di capacita**: `init --help` deve contenere `--force` (flag introdotto dalla riscrittura). E quello che fa `locateBinary`, e senza il quale l'estensione eseguiva la CLI vecchia, che scrive accanto al proprio eseguibile invece che nella cartella scelta - apparentemente non succedeva nulla.
- **Per provare l'estensione in un editor vero** (`@vscode/test-electron`) da dentro una sessione che gira gia in VS Code: va tolto `ELECTRON_RUN_AS_NODE` (e le `VSCODE_*`) dall'ambiente, altrimenti il binario parte come node e rifiuta ogni flag (`bad option: --user-data-dir`); e `--user-data-dir` deve essere **corto** (es. `/tmp/vsc-udd`), perche il socket IPC ci sta dentro e su macOS i socket Unix hanno un limite di ~104 caratteri.
