...
- **Cosa fa scattare un tag** (`git push --tags`, che fa sempre l'utente): `docker-publish.yml` pubblica l'immagine su ghcr.io e `vscode-publish.yml` pubblica sui **marketplace pubblici** (Open VSX + Visual Studio). `release.yml` e `go-release.yml` invece reagiscono alla *release pubblicata*, non al tag. Quindi il tag in locale è innocuo, il push del tag no: finché `vscode-extension-publish-metadata` è aperto (`publisher: "None"`) ogni tag pushato produce un publish fallito.
