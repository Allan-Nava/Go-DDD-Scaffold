...
- **Due namespace di tag, e non si sovrappongono**: `vX.Y.Z` è il tag del repo (uno per commit) e pushandolo fa partire solo `docker-publish.yml` → immagine su ghcr.io. `vscode-vX.Y.Z` è il tag dell'**estensione** e fa partire `vscode-publish.yml` → Visual Studio Marketplace + Open VSX. Separati di proposito: con un tag per commit, pubblicare l'estensione su `v*` significherebbe una versione nuova sui marketplace a ogni commit, e le versioni pubblicate sono immutabili e notificano gli utenti. `release.yml` e `go-release.yml` non reagiscono ai tag ma alla *release pubblicata*.
- **Per pubblicare l'estensione**: bump di `version` in `extensions/vscode/package.json`, commit, poi tag `vscode-v<stessa-versione>`. Il workflow **verifica che tag e `version` combacino** e fallisce se divergono. Senza il secret `VSCE_PAT` (e `OVSX_PAT`, opzionale) gli step di publish vengono **saltati**, non falliscono: un repo senza token non lascia job rossi.
